...
kelp_processed <- read_csv("data/manipulated/processed_kelp/kelp_processed.csv")
behavior_processed <- read_csv("data/manipulated/processed_behavior/behavior_processed.csv")
...
ggplot() +
    geom_point(data = predicted_kelp_mod, aes(x = group, y = predicted, color = x), position = position_dodge(0.5)) +
  geom_errorbar(data = predicted_kelp_mod, aes(x = group, ymin = conf.low, ymax = conf.high, color = x), position = position_dodge(0.5), width = .1) +
  
  geom_point(data = kelp_processed, aes(x = Urch_habitat_treatment, y = pcnt_grazed, color = Pred_treatment), position = position_dodge(0.5), alpha = 0.1) +
