...
kelp_mod_withsd <-  merge(x = predicted_kelp_mod, y = predicted_kelp_mod, kelp_processed_sd[ , "sd"], by = c("Pred_treatment","Urch_habitat_treatment"), all.x=TRUE)

  
  cbind(kelp_processed_sd,"Pred_treatment","Urch_habitat_treatment","sd"], 
        by = c("Pred_treatment", "Urch_habitat_treatment"))

merge(kelp_processed_sd["sd"], 
        by = c("Pred_treatment", "Urch_habitat_treatment"))
...
ggplot() +
  ...
  #model plot (mean GLMM value)
    geom_point(data = predicted_kelp_mod, 
               aes(x = group, y = predicted, color = Pred_treatment), 
               position = position_dodge(0.5)) +
  
  geom_errorbar(data = kelp_processed, 
                aes(x = Urch_habitat_treatment, ymin = conf.low, ymax = conf.high, color = Pred_treatment), 
                position = position_dodge(0.5), 
                width = .1) +
